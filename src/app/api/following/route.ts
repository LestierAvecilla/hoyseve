...
export async function GET(req: NextRequest) {
  ...
  try {
    // Fetch following — users that targetUser follows (followerId = userId)
    // With cursor pagination on createdAt DESC
    const conditions = cursor
      ? and(eq(follows.followerId, userId), lt(follows.createdAt, new Date(cursor)))
    ...
    // Determine isFollowing for each item (viewer follows this followed user?)
    ...
    console.error("[following GET]", err);
